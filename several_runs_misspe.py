...
#    varll = jnp.var(jnp.nansum(indiv_ll,axis=1))
...
def model_choice_noise(theta,n,J,meca_noise,dim,prng_key,n_preheat,n_iter):
    ...
    thetainit = jax.random.uniform(key=key_simu, shape=(models.parametrization.size,), minval=0.6, maxval=1.4)*thetatrue  

    wrong_noise = 1 - meca_noise

    wrong_res = algos.fisher_sgd(y=y,d=d,delta=1,meca_noise=wrong_noise,dim=dim,prng_key=key_estim,pre_heating=n_preheat,Nmax=n_iter,theta0=thetainit,optim_step='AdaGrad',factor=0.85)
    correct_res = algos.fisher_sgd(y=y,d=d,delta=1,meca_noise=meca_noise,dim=dim,prng_key=key_estim,pre_heating=n_preheat,Nmax=n_iter,theta0=thetainit,optim_step='AdaGrad',factor=0.85)
    ...
    bic_wrong, var_ll_wrong, ll_wrong = bic(theta_wrong,latent_wrong,wrong_noise,y,d,dim,N)
    bic_correct, var_ll_correct, ll_correct = bic(theta_correct,latent_correct,meca_noise,y,d,dim,N)

    return ResEstim(theta_wrong, theta_correct, ll_wrong, ll_correct, bic_wrong, bic_correct, y, t, z, wrong_res, correct_res)


import collections
ResEstim = collections.namedtuple("ResEstim",        ("theta_wrong","theta_correct","ll_wrong","ll_correct","bic_wrong","bic_correct","y","t","z","zw","zc"))
...
for exp in range(len(n_vec)):
    for i in range(Nsimus):
	    model_choice_noise(thetatrue,n_vec[exp],J_vec[exp],meca_noise,dim,key_all[i],500,3000)
