...
thetanames = [r"$\mu_{\lambda}$", r'$\mu_h$', r'$\sigma_{\lambda}$',
              r'$\sigma_h$', r'$\sigma_{\lambda,h}$', r'$\sigma_0$']
...
true_value['true_val_param'] = [
    0.7, 0.5, np.sqrt(0.07), np.sqrt(0.05), 0.5, 0.1]

true_value2 = pd.DataFrame({'true_value': theta2})
true_value2['variable'] = thetanames
true_value2['true_val_param'] = [1.75, 0.5,
                                 np.sqrt(0.175), np.sqrt(0.05), 0.5, 0.1]
...
def params_to_array(param):
    arr = jnp.concatenate([param.pop, param.indiv.mean_latent,
                          param.indiv.cov_latent[jnp.triu_indices(2)], jnp.array([param.var_residual])])
    return arr


def theta_to_params(theta):
    arr = jnp.array([theta[0], theta[1], jnp.log(1+jnp.exp(theta[2])), jnp.sqrt((jnp.log(1+jnp.exp(theta[3])) **
                    2+theta[4]**2)/2), theta[4]/jnp.sqrt(jnp.log(1+jnp.exp(theta[3]))+theta[4]), jnp.log(1+jnp.exp(theta[5]))])
    return arr
...
path = 'results/'  # Change this to the path where your results are stored
files = [f for f in os.listdir(path) if re.match(
    r'theta1_mecanoise1_d2_allres_n*', f)]
...
params_last[thetanames[3]] = np.sqrt(
    (np.log(1+np.exp(params_last[3]))**2+params_last[4]**2)/2)  # std h
# correlation between lambda and h
params_last[thetanames[4]] = params_last[4] / \
    (np.sqrt(2)*params_last[thetanames[3]])
params_last[thetanames[5]] = np.sqrt(
    np.log(1+np.exp(params_last[5])))  # residual std

params_last = pd.melt(params_last.drop(
    ['rep', 'iter', 0, 1, 2, 3, 4, 5], axis=1), id_vars=['N', 'd', 'noise'])
...
rmse = params_last.groupby(['N', 'variable'], as_index=False).agg({
    'bias2': ['mean'], 'value': ['var']})
...
rmse.columns = ['index', 'N', 'Parameter',
                'Bias (squared)', 'Variance', 'mse', 'relmse']
...
params2_last[thetanames[3]] = np.sqrt(
    (np.log(1+np.exp(params2_last[3]))**2+params2_last[4]**2)/2)  # std h
params2_last[thetanames[4]] = params2_last[4] / \
    (np.sqrt(2)*params2_last[thetanames[3]])  # correlation between lambda and h
params2_last[thetanames[5]] = np.sqrt(
    np.log(1+np.exp(params2_last[5])))  # residual std


params2_last = pd.melt(params2_last.drop(
    ['rep', 'iter', 0, 1, 2, 3, 4, 5], axis=1), id_vars=['N', 'd', 'noise'])
...
rmse2 = params2_last.groupby(['N', 'variable'], as_index=False).agg({
    'bias2': ['mean'], 'value': ['var']})
...
rmse2.columns = ['index', 'N', 'Parameter',
                 'Bias (squared)', 'Variance', 'mse', 'relmse']
...
g = sns.FacetGrid(rmse_all, col="Parameter", hue="Parameter set",
                  sharey=False, col_order=thetanames, col_wrap=3)
g.map(sns.lineplot, "N", "mse", marker='o')
for item, ax in g.axes_dict.items():
    ax.grid(False, axis='x')
    ax.set_title(item)
    ax.secondary_xaxis('top', functions=(utils.invert, utils.invert))

# relative MSE
sns.set_style("whitegrid")
g = sns.FacetGrid(rmse_all, col="Parameter", hue="Parameter set",
                  sharey=False, sharex=False, col_order=thetanames, col_wrap=3)
...
for item, ax in g.axes_dict.items():
    ...
    secax = ax.secondary_xaxis('top', functions=(utils.invert, utils.invert))
    ...
    g.map_dataframe(utils.plot_boxplot_with_twinx_axis)
