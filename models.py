...
def comp_log_likelihood_rows(theta, z, y, d, delta=1, meca_noise=1., dim=2):
    ...
    varz = jnp.log(jnp.diag(1/p.indiv.mean_latent) @ p.indiv.cov_latent @
                   jnp.diag(1/p.indiv.mean_latent) + jnp.ones((dim_latent, dim_latent)))
...
def cond_log_likelihood_rows(theta, z, y, d, delta=1, meca_noise=1., dim=2):
    ...
    dy2 = (y - my)**2/vary
    log_likli_obs = - 0.5 * J * \
        jnp.log(2 * jnp.pi) - 0.5 * jnp.sum(jnp.log(vary),
                                            axis=1) - 0.5 * jnp.sum(dy2, axis=1)
...
def log_likelihood_is(theta, latent, y, d, delta=1, N=1000, meca_noise=0., dim=2, prng_key=0):
    ...
    logz = jnp.array([logzstd[i, :, :] * sd_is[i, :] + muz_is[i, :]
                     for i in range(N)])
    ...
    for i in tqdm(range(N)):
        ...
        ypred, meany, vary = model(
            lamb=lamb, h=h, delta=delta, var_res=p.var_residual, d=d, meca_noise=meca_noise, dim=dim)

        # quantities that are averaged by IS
        # 1. conditional likelihood of the observation given the latent

        indiv_cond_lik = jnp.nanprod(
            jax.scipy.stats.norm.pdf((y-meany)/jnp.sqrt(vary)), axis=1)
...
def mh_step_gibbs_prior_prop(mean_proposal, sigma_proposal, current_ar, it, theta, z, y, d, delta, meca_noise, dim, prng_key):
    ...
    for k in range(dim_latent):
        ...
        z_propo = z.at[:, k].set(jax.random.normal(
            key1, shape=(n,)) * sigma_proposal[k] + mean_proposal[k])
        log_likli_propo = cond_log_likelihood_rows(
            theta, z_propo, y, d, delta, meca_noise, dim)
...
def mhrw_step_gibbs_adaptative(sigma_proposal, current_ar, it, theta, z, y, d, delta, meca_noise, dim, prng_key):
    ...
    accept, z, log_likli = mhrw_step_gibbs(
        theta, z, y, d, delta, meca_noise, dim, sigma_proposal, prng_key)
...
def model(lamb, h, delta, var_res, d, meca_noise=1., dim=2, prng_key=0):
    ...
    cE = jax.lax.cond(dim == 2,
                      ...
                      lambda dim: (6*jnp.sqrt(dim) - jnp.pi +
                                   jnp.log(3650401+2107560*jnp.sqrt(dim)))/24,
                      dim)
    cV = dim/3
...
def simu_data(theta, n, d=20, delta=1, meca_noise=1., dim=2, prng_key=0):
    ...
    varz = jnp.log(jnp.diag(1/p.indiv.mean_latent) @ p.indiv.cov_latent @
                   jnp.diag(1/p.indiv.mean_latent) + jnp.ones((dim_latent, dim_latent)))
    muz = jnp.log(p.indiv.mean_latent) - 0.5*jnp.diag(varz)
    logz = jax.random.normal(key=key1, shape=(
        n, dim_latent)) @ jnp.linalg.cholesky(varz).T + muz
    z = jnp.exp(logz)
    ...
    y, my, vary = model(lamb, h, delta, p.var_residual,
                        d, meca_noise, dim, prng_key)
