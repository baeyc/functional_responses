...
def theta_to_params(theta):
    arr = jnp.array([theta[0], theta[1], jnp.log(1+jnp.exp(theta[2]))**2, (jnp.log(1+jnp.exp(theta[3])) **
                    2+theta[4]**2)/2, (jnp.log(1+jnp.exp(theta[2]))*theta[4])/jnp.sqrt(2), jnp.log(1+jnp.exp(theta[5]))])
    return arr


thetanames = [r"$\mu_{\lambda}$", r'$\mu_h$', r'$\sigma_{\lambda}^2$',
              r'$\sigma_h^2$', r'$\sigma_{\lambda,h}$', r'$\sigma_0^2$']
...
files = [f for f in os.listdir(path) if re.match(
    r'modchoice_mecanoise1_d2_bicwrong*', f)]
...
bp = sns.boxplot(x='N', y='BIC', hue='Model',
                 gap=0.2, fliersize=3, data=bic_all)
...
theta_correct[thetanames[3]] = (
    np.log(1+np.exp(theta_correct[3]))**2+theta_correct[4]**2)/2
theta_correct[thetanames[4]] = (
    np.log(1+np.exp(theta_correct[2]))*theta_correct[4])/np.sqrt(2)
...
theta_wrong[thetanames[3]] = (
    np.log(1+np.exp(theta_wrong[3]))**2+theta_wrong[4]**2)/2
theta_wrong[thetanames[4]] = (
    np.log(1+np.exp(theta_wrong[2]))*theta_wrong[4])/np.sqrt(2)
theta_wrong[thetanames[5]] = np.log(1+np.exp(theta_wrong[5]))


theta_correct = pd.melt(theta_correct.drop(
    ['rep', 0, 1, 2, 3, 4, 5], axis=1), id_vars=['N', 'd', 'noise'])
theta_wrong = pd.melt(theta_wrong.drop(
    ['rep', 0, 1, 2, 3, 4, 5], axis=1), id_vars=['N', 'd', 'noise'])
...
g = sns.FacetGrid(dtheta_all, col="variable",
                  sharey=False, col_order=thetanames)
g.map_dataframe(sns.boxplot, x="N", y="value", hue="Model",
                palette=palette, dodge=True, gap=0.1, fliersize=2, width=0.5)
for ax, pos in zip(g.axes.flat, true_value['true_value']):
    ax.axhline(y=pos, color='r')
g.add_legend(title='')
sns.move_legend(g, bbox_to_anchor=(0.5, -0.1),
